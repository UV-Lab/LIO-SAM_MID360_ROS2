...
void coreImportTest(void) { cout << "scancontext lib is successfully imported." << endl; }
...
std::pair<int, float> SCManager::detectLoopClosureID(void) {
    ...
    return detectLoopClosureIDBase(curr_key, curr_desc);
}

std::pair<int, float> SCManager::detectLoopClosureID(pcl::PointCloud<SCPointType>& _scan_down) {
    Eigen::MatrixXd sc = makeScancontext(_scan_down);  // v1
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(sc);
    // Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(sc);
    std::vector<float> polarcontext_invkey_vec = eig2stdvec(ringkey);
...
std::pair<int, float> SCManager::detectLoopClosureIDBase(std::vector<float>& curr_key, Eigen::MatrixXd& curr_desc) {
    int loop_id{-1};  // 初始化为-1，-1表示没有环路闭合（等同于LeGO-LOAM的变量"closestHistoryFrameID"）
    ...
    if ((int)polarcontext_invkeys_mat_.size() < NUM_EXCLUDE_RECENT + 1) {
        ...
        return result;  // 如果候选项不足，则提前返回
    }

    // tree_重构（不是每次都需要）
    if (tree_making_period_conter % TREE_MAKING_PERIOD_ == 0)  // 为了节省计算资源
    {
        // TicToc t_tree_construction;  // 计时开始

        polarcontext_invkeys_to_search_.clear();
        polarcontext_invkeys_to_search_.assign(polarcontext_invkeys_mat_.begin(),
                                               polarcontext_invkeys_mat_.end() - NUM_EXCLUDE_RECENT);  // 去除最近的一些帧，避免误匹配

        polarcontext_tree_.reset();
        polarcontext_tree_ = std::make_unique<InvKeyTree>(PC_NUM_RING /* dim */, polarcontext_invkeys_to_search_, 10 /* max leaf */);
        // tree_ptr_->index->buildIndex(); // 在InvKeyTree的构造函数中已经调用，用于构建KD树（具体参见nanoflann和KDtreeVectorOfVectorsAdaptor）
        // t_tree_construction.toc("Tree construction");  // 计时结束
    }
    tree_making_period_conter = tree_making_period_conter + 1;

    double min_dist = 10000000;  // 初始化为一个很大的值
    int nn_align = 0;
    int nn_idx = 0;
    ...
    // t_calc_dist.toc("Distance calc");  // 计时结束
    ...
    if (min_dist < SC_DIST_THRES) {
        ...
        //     << "\tyaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl; // 如果发现环路闭合，输出距离和旋转差异
    } else {
        // std::cout.precision(3);
        // cout << "[Not loop] Nearest distance: " << min_dist << " btn " << polarcontexts_.size() - 1 << " and " << nn_idx
        //     << "\tyaw diff: " << nn_align * PC_UNIT_SECTORANGLE << " deg." << endl; // 如果没有发现环路闭合，输出距离和旋转差异
    }

    // TODO: 返回nn_align（即，yaw diff）
    float yaw_diff_rad = deg2rad(nn_align * PC_UNIT_SECTORANGLE);
    std::pair<int, float> result{loop_id, yaw_diff_rad};  // 返回环路ID和yaw差异（弧度）
